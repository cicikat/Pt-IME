...
当前实现只记住单个已存在候选的哈希，不会把一次输入事务中连续选择的“角”与“色”合成为用户词。因此重复输入不会生成“角色”，简拼 `js` 也只能返回静态词库中的其他词。
...
5. 一次完整输入中依次选择“角”“色”后，立即学成用户词“角色”。
6. 学成后：`juese` 第一候选为“角色”；`js` 中“角色”位于第二候选。
...
本项验收：新增真实词库审计脚本或等价只读检查，验证 `lexicon.db` 中 `xuan/juese/js` 的事实；新增 JVM 固定 fixture 回归覆盖上述顺序，不依赖真机肉眼猜测。
...
4. 完成一次“角”＋“色”的选择后，立即验证 `juese` 第一、`js` 第二。
