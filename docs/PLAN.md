...
- [x] **2026-08-11 P0 候选分层与 v2 造词**：显式候选来源层级保护合法单音节精确项；新增 `candidate_memory_v2` 增量迁移和组件 ID 词记忆，覆盖“角色”→ `角色`/`juese`/`js`，词库版本不匹配 fail closed；静态词库只读审计脚本位于 `tools/check_lexicon_facts.py`
...
## 2026-09-11 — 整句过度拆音节修复

- [x] 在引擎内部计算输入前缀的最少合法音节数，对纯全拼路径的额外音节扣分，Beam 裁剪与最终排序使用同一代价；保留整词优先、简拼和手动分隔符。
- 回归：从随包静态词库提取 692 条测试词条，覆盖 `xianluomao` / `meiliaoba` 前三候选不拆成四五字、`xi'an'hao` 的显式边界，以及 `xianhao` 的“西安”词典歧义。关闭新代价时新增回归确实失败。
- 验证：`gradlew.bat assembleDebug testDebugUnitTest` 通过，50 项单测通过；真机验收待用户安装确认。此项修复音节结构，不保证“暹罗猫”或“没聊吧”首选；上下文同音词排序仍有限。

## 2026-09-11 — 设置、面板与基础美化

- [x] 回传配置显式保存、HTTP 与公网穿透选项、后台连接测试及错误反馈；设置页独立导航和固定返回键。（前一轮已构建提交。）
- [x] 系统输入法子类型标记中文拼音；符号和 Emoji 统一工具栏、选中标记、分类和退格键，最近内容仅重进刷新。405 提示完整接收路径。（已构建提交。）
- [x] 基础美化：图片/动图背景导入、拖动与缩放裁剪、边界限制、按键透明度、压暗、TTF/OTF 导入选择、Debug 可选 Zpix 和即时预览；主题包后台导入与反馈。
- 验证：`assembleDebug testDebugUnitTest lintDebug` 通过，54 项单测无失败；Debug 包含本机 Zpix，已成功覆盖安装。手机持续在使用，最终页面手感、动图与字体效果由用户确认，不将安装成功视为验收完成。
