...
> 当前草稿协议以 [三小时草稿 v2](docs/draft_sync_v2.md) 为准。2026-09-09 语音改为内置 sherpa-onnx 中英双语流式模型，约 198 MB（原计划 40–70 MB 估算不适用）；录音/解码走独立线程、按需加载、闲置 60 秒释放，PCM 不落盘、不上传。候选更新保留上一帧但禁止选择旧 revision；主题/布局文件及背景图加载移出主线程。下述日报聚合仍为规划。
...
纯全拼 Beam 路径以静态词条 initials 长度累计音节数，并与当前原串前缀的最少合法音节数比较；每个额外音节扣 3000 分。最少音节数通过有界动态规划计算，显式分隔符为硬边界。此软代价同时用于中途裁剪和最终分数，避免 mao → ma + o、liao → li + a + o 依赖高频字获利；不删除合法歧义，也不改变完整词条来源层级、简拼评分或用户记忆。系数属于启发式，后续应以固定回归语料和真机反馈调整。

### 2026-09-11 基础外观与面板

ThemeRepository 的 appearance StateFlow 保存本机背景、归一化裁剪、按键不透明度、蒙层和字体覆盖项；图片、字体与主题包导入及解码均在 IO 线程。KeyboardBackground 统一用于预览、裁剪与 IME，内部使用有尺寸上限的 ImageDecoder，Drawable 由 View 管理回调并随窗口可见性启停。BackgroundCrop 是纯 Kotlin 几何模型，边界单测保证横竖视口不露底。字体只注入键盘 TextStyle，不改变设置页字体。

符号/Emoji 共享顶层工具栏，分类栏在其下方；入口可切换和再次点击退出，选中标记仅用于这两个入口。最近列表在进入面板时取快照，点击只更新持久仓库。底部右侧保留退格键，复用字母键盘的长按连续删除。
